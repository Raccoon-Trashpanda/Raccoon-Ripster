...
а нужен был нишевый Ambient Drum & Bass. `artist_xref._pick_exact` осторожен, но
...
def artist_service_ids(mbid: str) -> dict:
    """{service: {'url':…, 'id':…}} из url-rels артиста. Пусто — ссылок нет."""
    mbid = (mbid or "").strip()
    if not mbid:
        return {}
    _load()
    ...
    for rel in data.get("relations", []):
        ...
        for host, name in _SVC_HOST.items():
            if host in low and name not in svc:
                m = re.search(r"/(?:artist|user)/([A-Za-z0-9._-]+)", url)
                svc[name] = {"url": url, "id": m.group(1) if m else ""}
                break
...
def resolve_service_id(name: str, service: str, genre_hint: str = "") -> Optional[dict]:
    ...
    витрину. Нет MB-записи / нет ссылки на этот сервис → None, и вызывающий
    падает на прежний name-search (`artist_xref`).
    """
