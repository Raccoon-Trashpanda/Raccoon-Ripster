...
async def send_report(cfg: dict, base_dir: Path, version: str, note: str = "") -> dict:
    ...
    # через туннель, а кладётся на то же самое место. Развилка нужна по роли, а
    # не по токену: петля «свой же /api/telemetry/report» отдаёт владельцу
    # чужую для него природу записи (публичный ярус с лимитами) там, где он
    # пишет у себя дома. Проверяем именно роль, а не адрес: forwarded_enabled()
    # исключает приёмник из отправки по той же причине (см.
    # telemetry.forwarding_enabled).
