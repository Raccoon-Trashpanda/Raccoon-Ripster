...
def order_pos(accounts: list, i: int) -> int:
    """Позиция учётки `i` в порядке предпочтения. Для ВЫКЛЮЧЕННОЙ — хвост, а не
    исключение.

    `order_indices` намеренно НЕ включает выключенные учётки. Панели же
    перебирают ВСЕ и раньше делали `order_indices(...).index(i)` — для
    выключенной это `ValueError`, и он ронял НЕ строку, а ВЕСЬ ответ: 500 на
    `/api/<svc>/accounts`, а в настройках вместо пула пустота. 18.09.2026 у
    Qobuz одна учётка стояла `enabled: false` — и пропал весь пул, выглядело как
    «удалились токены». Ошибка была во всех пяти пулах сразу (8 одинаковых мест),
    потому и чинится ОДНОЙ функцией, а не копией в каждом.
    """
    order = order_indices(accounts)
    try:
        return order.index(i)
    except ValueError:
        return len(order) + i      # выключенные уходят в конец, но ОСТАЮТСЯ видимы
