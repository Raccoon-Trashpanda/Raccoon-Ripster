...
FALLBACK: dict[str, str] = {
    ...
    "console.pr_fb_miss": "🔁 {svc}: {title} — на других витринах его нет: этот релиз есть только на {svc}",
    "console.pr_fb_no_id": "🔁 {svc}: у релиза нет ISRC/UPC — подтвердить идентичность нечем, вслепую не ищу",
    # Страж понижения качества (24.09.2026: FLAC просили, витрина отдала AAC —
    # раньше это была жёлтая строка в логе при зелёной плитке «готово»).
    "console.qd_retry": "⟳ {svc} отдал {got} вместо {want} — обновляю сессию и качаю заново (lossy-файлы убраны)",
    "console.qd_fb_try": "🔁 {svc} отдала пониженное качество ({got} вместо {want}) — беру то же на другой витрине",
    "console.qd_downgraded": "⚠ Скачано в пониженном качестве: {got} вместо {want} — ни одна витрина не отдала выше",
    "history.qd_downgraded": "скачано в пониженном качестве: {got} вместо {want}",
    ...
    "preorder.notify_slipped":  "🕝 Релиз «{title}» снова отодвинулся: ждём {date} (попытка {n} из {max}).",
    ...
    # 24.09.2026: отдельный диагноз для отдельной причины. Apple прямо пишет, что
    # аккаунт закреплён не за той страной, где подписка, — это НЕ «релиза нет» и
